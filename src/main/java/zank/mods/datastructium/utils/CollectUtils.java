...
public interface CollectUtils {
    ...
    static <T> List<T> ofList(Collection<T> elements) {
    ...
    static <K, T> Function<K, ArrayList<T>> keyIgnoredNewArrayList() {
        return (Function) KEY_IGNORED_NEW_ARRAY_LIST;
    }

    /**
     * empty list -> a singleton instance of empty list
     * <p>
     * list with only one element -> a new singleton list
     * <p>
     * other lists -> itself
     */
    static <T> List<T> reduceSmallList(List<T> list) {
        if (list.isEmpty()) {
